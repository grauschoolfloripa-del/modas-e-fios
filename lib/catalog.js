...
export async function getCatalogAsync(type) {
  try {
    const { createClient } = await import("@/lib/supabase/server");
    const supabase = await createClient();
    if (!supabase) return type ? CATALOG.filter((p) => p.type === type) : CATALOG;

    let query = supabase.from("products").select("*").eq("published", true);
    if (type) query = query.eq("type", type);
    const { data, error } = await query.order("created_at", { ascending: false });

    if (error) return type ? CATALOG.filter((p) => p.type === type) : CATALOG;
    return data.map(mapDbProduct);
  } catch {
    return type ? CATALOG.filter((p) => p.type === type) : CATALOG;
  }
}
